#' MetaG.MetaB.link function
#'
#' This function allows you to identify MetaG - MetaB module pairs with biological links.  
...
#' @param  MetaG_module.feature_file   KEGG_modules.tab file, source  ????
#' @param  MetaG_quantity_file   metagenomic quantification file on the feature level
#' @param  KO2METABO_file   KO2METABO.lists.RData generated from ko2metabo() function
...
#' @param  ACME.p.co   cutoff of ACME p_value to identify potentially linked MetaG - MetaB modules
#' @param  output.dir   default "biological.links"
#' @param  output.prefix  
#' @param  log.file    default "MetaG.MetaB.link.log"
#' 
#' @examples
#' MetaG.MetaB.link(mediation.res = MetaG.MetaB.NUE_medres, 
...
MetaG.MetaB.link <- function(
  ...
  MetaG_module.feature <- fread(MetaG_module.feature_file, data.table = F, header = F, col.names = c("Module","Description","Features")) 
  
  load(KO2METABO_file)
  
  MetaB_module.feature <- fread(MetaB_module.feature_file, data.table = F,col.names =c("Feature","Module"))
  
  MetaB.dat <- data.frame(fread(MetaB_quantity_file, data.table = F), row.names = 1) %>% t() %>% as.data.frame()
  
  metabo.KEGGmodule.match <- fread(metabo.KEGGmodule.match_file,data.table = F)
...
\nError message returned by 'cmapR::parse.gctx()':\n\n", dataset, '\n\n')
  ...
  for(mdp in MetaG.MetaB.modPairs){  # MetaG.MetaB.modPairs
    ...
    for(gf in metag.ftr){
      ...
      if(T){
        bf = tmp[1]
        link = c(paste(parts[1],parts[2],sep = "_"), gf, NA, NA, bf )
        names(link) <- c("MetaG.MetaB_modulePair", "MetaG.feature","bridging.MetaG.feature", "bridging.MetaB.feature", "MetaB.feature")
        ConfirmedLink <- T
    ...
    for(bf in metab.ftr){
      ...
      if(any(gms == metag.md)){
        link = c(paste(parts[1],parts[2],sep = "_"),NA, NA, NA,bf )
        names(link) <- c("MetaG.MetaB_modulePair", "MetaG.feature","bridging.MetaG.feature", "bridging.MetaB.feature", "MetaB.feature")
        ConfirmedLink <- T
        links <- bind_rows(links,link)
        remove(link)
      ...
      for(bf1 in tmp){
        ...
        for(bf2 in metab.ftr){
          ...
          if(length(intersect(bf1.keggM, bf2.keggM)) > 0) {
            link = c(paste(parts[1],parts[2],sep = "_"), gf1, NA, bf1, bf2 )
            names(link) <- c("MetaG.MetaB_modulePair", "MetaG.feature","bridging.MetaG.feature", "bridging.MetaB.feature", "MetaB.feature")
            ConfirmedLink <- T
            links <- bind_rows(links,link)
            remove(link)
      ...
      for(gf1 in tmp){
        ...
        for(gf2 in metag.ftr){
          ...
          if(length(intersect(gf1.Gms, gf2.Gms)) > 0){
            link = c(paste(parts[1],parts[2],sep = "_"), gf2, gf1,  NA, bf1 )
            names(link) <- c("MetaG.MetaB_modulePair", "MetaG.feature","bridging.MetaG.feature",  "bridging.MetaB.feature","MetaB.feature")
            ConfirmedLink <- T
            links <- bind_rows(links,link)
            remove(link)
  ...
  write.table(links, file = paste(output.dir,"/",output.prefix,"MetaG.MetaB.modules.linked.txt",sep = ""), sep = "\t", quote = F, row.names = F)
