#' ssGSEA2 function
#'
#' This function allows you to do ssGSEA2 
...
#' @param  sample.norm.type  sample normalization method "rank", "log", "log.rank", "none"  
#' @param  weight  when weight==0, all genes have the same weight; if weight>0, actual values matter, and can change the resulting score
#' @param  statistic test statistic "area.under.RES", "Kolmogorov-Smirnov"
#' @param  output.score.type "NES", "ES"
#' @param  nperm   number of random permutations for NES case, default 200
#' @param  combine.mode      "combine.off", "combine.replace", or "combine.add" "combine.off" do not combine _UP and _DN versions in a single score. "combine.replace" combine _UP and  _DN versions in a single score."combine.add" combine  _UP and  _DN versions in a single score and add it but keeping the individual _UP and _DN versions.
#' @param  min.overlap    default 10
#' @param  correl.type     correlation type: "rank", "z.score", "symm.rank"  default "rank"
#' @param  global.fdr      if TRUE calculate global FDR; else calculate FDR sample-by-sample, default F
#' @param  extended.output   if TRUE the result GCT files will contain statistics about gene coverage etc.  default T.
#' @param  par  if TRUE use multicore 
#' @param  spare.cores   number of spared cores left if par is TRUE
#' @param  export.signat.gct  if TRUE gct files with expression values for each signature will be generated, default T
#' @param  param.file 
#' @param  log.file  default 'run.log'
#' @param  outputDir  default "metaG_DR"
#' 
#' @examples
#' ssGSEA2(input file, output.prefix="metagenomic", min.overlap = 2)
...
\nError message returned by 'cmapR::parse.gctx()':\n\n", dataset, '\n\n')
  ...
  input.file.prefix <-  sub('^(.*)\\.gct$', '\\1', basename(input.ds))
  ...
  keep.idx <- which(size.ol.G >= min.overlap)  # if a module have more than min.overalp genes detected, keep it 
  ...
  V.GCT@src <- gct.src
  fn <- paste(outputDir,"/",output.prefix, '-scores.gct', sep='')
  #V.GCT@fname <- fn
  ...
  P.GCT@src <- gct.src
  fn <- paste(outputDir,"/", output.prefix, '-pvalues.gct', sep='')
  #P.GCT@fname <- fn
  ...
  F.GCT@src <- gct.src
  fn <- paste(outputDir,"/",output.prefix, '-fdr-pvalues.gct', sep='')
  #F.GCT@fname <- fn
  ...
  fn <- paste(outputDir,"/",output.prefix, '-combined.gct', sep='')
  #ALL.GCT@fname <- fn
