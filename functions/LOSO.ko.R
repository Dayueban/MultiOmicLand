...
#' 
#' @param  geneDepth_df  input data frame of gene quantification, with rownames being gene ids and columns being samples; gene ids should be formated in "scaffold_number"
...
#'         bin.species_file = "LOSO/bin_species.txt")
#' 
#' 
#' 
...
LOSO.ko <- function(
  ...
  geneDepth_df <- fread(geneDepth_file) %>%   # ,nrows = 300 to test
    dplyr::filter(V1 %in%  gene.KO_df$Gene) %>% 
  ...
  for(species in uniqSpecies){
    ...
    fnn = paste("LOSO_ko.abund/ko.abund_rm.", species, ".gct", sep = "")
    gct.tmp@src <- fnn
