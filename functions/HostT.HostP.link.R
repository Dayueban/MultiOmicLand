...
#'
#' This function allows you to identify biological linkes between HostT modules and HostP features.  
...
#' @param  HostT_quantity_input   HostT quantification on the feature level as a file path, or as a dataframe with rownames being features and colnames being samples
#' @param  HostP_quantity_input   HostP quantification on the feature level as a file path, or as a dataframe with rownames being features and colnames being samples
...
#' @param  HostP_protein.gene_file    protein names and corresponding gene names, sourced from  ????????????????
#' @param  Pthway2Gene_file  Pathway information file (.gmt file) sourced from ????????????
#' @param  ACME.p.co    cutoff of ACME p value to define MetaB-HostT module pairs with potential biological links
#' @param  output.dir   default "biological.links"
#' @param  output.prefix 
#' @param  log.file    default "HostT.HostP.link.log"
#' 
#' @examples
#' MetaG.MetaB.link(mediation.res = MetaB.HostT.NUE_medres, 
...
HostT.HostP.link <- function(
  mediation.res = HostT.HostP.NUE_medres,
  ...
  library(dplyr)
  # impport host t data
  if(class(HostT_quantity_input) == "character"){
    if(grepl("\\.gct$", HostT_quantity_input)) m1 <- parse.gctx(HostT_quantity_input)@mat  else m1 <- data.frame(fread(HostT_quantity_input), row.names=1)
  }else{ m1 = HostT_quantity_input }
  ...
  if(class(HostP_quantity_input) == "character"){
    if(grepl("\\.gct$", HostP_quantity_input)) m2 <- parse.gctx(HostP_quantity_input)@mat  else m2 <- data.frame(fread(HostP_quantity_input), row.names=1)
  }else{
    m2 = HostP_quantity_input
  ...
  temp <- fread(HostP_protein.gene_file, data.table = T) %>% dplyr::select(Target, GeneName) %>% unique()
  ...
  PTWY_list <- Read.GeneSets.db2(Pthway2Gene_file)
  ...
  write.table(links, file = paste(output.dir,"/",output.prefix,"HostT.module_HostP.protein.linked.txt",sep = ""), sep = "\t", quote = F, row.names = F)
