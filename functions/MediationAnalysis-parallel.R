...
#' @param  threads   number of threads to perform parallel calculation
#' 
#' @examples
...
MediationAnalysis_parallel <- 
  function(
    ...
    ## parallel loop
    Mod.pairs <- expand.grid(Treat.omic.sigModules, Mediator.omic.sigModules)
    
    Mediation.res <- foreach(i = 1:nrow(Mod.pairs),  .packages='doParallel') %dopar% {
      
      md1 = Mod.pairs$Var1[i]; md2 = Mod.pairs$Var2[i]
      ...
      m1 <- m1[match(meta_df$SampleID, rownames(m1)),]
      m2 <- m2[match(meta_df$SampleID, rownames(m2)),]
      
      
      dat <- cbind.data.frame(meta_df,
                       Treat = m1[,md1],
                       Mediator = m2[,md2],
                       stringsAsFactors=F)
      ...
      dat <- dat[!is.na(dat$Y) & !is.na(dat$Treat) & !is.na(dat$Mediator),]
      ...
      if(class(testModel) != 'try-error' ){
        ...
        vec = c(id,NA,NA,NA,NA)
        names(vec) <- c("Treat_Mediator_Y", "ACME.p", "ADE.p", "prop.mediated", "spearman.r")
      }
     
      
      
      #Mediation.results <- dplyr::bind_rows(Mediation.results, vec)
      vec
     # Mediation.results
    }
    ...
    names(Mediation.res) <- seq(1,length(Mediation.res),1)
    Mediation.res <- data.frame(Mediation.res) %>% t() %>% data.frame()
    ...
    write.table(Mediation.res, file = paste(outputDir, "/", Treat.omic,"_affects_", Y, "_through_", Mediator.omic,"_parallel.txt",sep = "" ),
                quote = F, row.names = F, sep = "\t")
    
    return(Mediation.res)
  }